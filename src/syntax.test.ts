...
import { parity } from "@astronautlabs/st291";
...
describe("Packet", it => {
    ...
    it.only('properly deserializes a MultipleOperationMessage', async () => {
        let buf = Buffer.from([
            0xFF, 0x24, 0x14, 0x1D, 0x25, 0xBA, 0x2F, 0xFB, 
            0xFE, 0x00, 0x89, 0x20, 0x08, 0x01, 0x01, 0x80, 
            0x20, 0x08, 0x01, 0x01, 0x98, 0x26, 0x07, 0x69, 
            0xC8, 0x80, 0x20, 0x04, 0x05, 0x01, 0x40, 0x60, 
            0x04, 0x39, 0x01, 0x98, 0x26, 0x07, 0x69, 0xC8, 
            0x95, 0xAC, 0x38, 0x3E, 0xA0, 0x82, 0x66, 0x08, 
            0x02, 0x00, 0x40, 0x68, 0x10
        ]);

        let packet = await ST2010.Packet.deserialize(buf);
        expect(packet.payload.length).to.equal(36);
        ...
        if (message instanceof SCTE104.elements.MultipleOperationMessage) {
            ...
            expect(message.messageSize).to.equal(36);
    ...
    it.only('properly serializes a MultipleOperationMessage', async () => {
        let buf = Buffer.from([
            0xFF, 0x24, 0x14, 0x1D, 0x25, 0xBA, 0x2F, 0xFB, 
            0xFE, 0x00, 0x89, 0x20, 0x08, 0x01, 0x01, 0x80, 
            0x20, 0x08, 0x01, 0x01, 0x98, 0x26, 0x07, 0x69, 
        ...
        let serializedBuf = packet.serialize(undefined, undefined, true);
        expect(serializedBuf).to.eql(buf);

        // Check that parity was encoded correctly
        let unserialized = await ST2010.Packet.deserialize(buf);
        expect(unserialized.userDataCountParity).to.equal(parity(unserialized.userDataCount));
        expect(unserialized.didParity).to.equal(parity(unserialized.did));
        expect(unserialized.sdidParity).to.equal(parity(unserialized.sdid));
    });
});